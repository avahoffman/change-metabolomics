...
compile_and_fit_normal_model <- function(df, responsevar, iter = 100000){
  ...
  modeldat = list(
    'N' = nrow(df),
    'PC' = responsevar,
    'spp' = as.numeric(df$spp) - 1, ## want zeros and ones
    'Nit' = df$nitrogen
  )
  fit <-sampling(
    comp.normal,
...
plot_main_effects <-  function(fit){
  ...
  param.plot <- mcmc_dens(
    posterior,
    pars = c("Species effect", "Nitrogen effect", "Interaction effect"),
...
write_model_statistics <- function(fit, name){
  ...
  model_stats <-
    cbind(summary(fit)$summary[, 1, drop = F],
          summary(fit)$summary[, 4, drop = F],
          summary(fit)$summary[, 8, drop = F],
          summary(fit)$summary[, 10, drop = F])
  write.csv(model_stats, file = paste("output/stanmodel_results_", name, "_stats.csv", sep=""))
  
  ## Pr values
  print("Calculating and writing Pr values..")
  Pr_vals <-
    rbind(model_stats["B0", ], model_stats["B1", ], model_stats["B2", ], model_stats["B3", ])
  Pr_vals <- Pr_vals[, -4] ## drop rhat
  ...
  write.csv(Pr_calc, file = paste("output/stanmodel_results_", name, "_pr.csv", sep=""))
}
...
plot_posterior_checks <- function(fit, responsevar, name){
  ...
  np1 <- ppc_dens_overlay(responsevar, yrep[1:400, ]) + 
    theme_minimal(base_size = 20) +
    xlab(name)
  ggsave(np1, file = paste("output/stanmodel_results_", name, "_predictivecheck1.pdf", sep=""))
  pdf(file = paste("output/stanmodel_results_", name, "_predictivecheck2.pdf", sep=""))
  hist(responsevar,
       prob = T,
       breaks = 10,
       main = name)
...
model_first_three_pcs <- function(){
  # Load data
  dat <- read.csv(file = "output/pca_scores_predictors.csv", header = T)
  
  # PC1
  fit <- compile_and_fit_normal_model(dat, responsevar = dat$PC1)
  plot_main_effects(fit)
  write_model_statistics(fit, name = "PC1")
  plot_posterior_checks(fit, responsevar = dat$PC1, name = "PC1")
  
  # PC2
  fit <- compile_and_fit_normal_model(dat, responsevar = dat$PC2)
  plot_main_effects(fit)
  write_model_statistics(fit, name = "PC2")
  plot_posterior_checks(fit, responsevar = dat$PC2, name = "PC2")
  
  # PC3
  fit <- compile_and_fit_normal_model(dat, responsevar = dat$PC3)
  plot_main_effects(fit)
